...
const page = () => {
  ...
  const { useSavedPostList, useDetailPost } = usePostModule();
  const {
    data: saved,
    isFetching,
    isLoading,
...
console.log(saved, 'sav')
  return (
    <div>
      <div className="p-8">
        <h2 className="text-teal-900 font-semibold text-2xl">Saved Post !</h2>
      </div>
      <div className="w-screen h-screen flex flex-col justify-start items-center px-6 ">
        <Box sx={{ width: "100%" }}>
          <Masonry columns={{ xs: 2, sm: 3, md: 6 }} spacing={2}>
            {saved?.data?.map((item: any, index: any) => {
              return (
                <Link href={`/user/post/detail/${item.post_id.id}`} key={index}>
                  <div
                    className="bg-white"
                    onClick={() => {
